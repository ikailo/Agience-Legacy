...
public static class HostBuilderExtensions
{
    public static IHostApplicationBuilder AddAgienceHost(this IHostApplicationBuilder appBuilder,
          ...
          string? authorityUriOverride = null,
          ...
          )
    {
        ...
        appBuilder.Services.AddSingleton(x => new Authority(authorityUri, x.GetRequiredService<Broker>(), x.GetRequiredService<ILogger<Authority>>(), authorityUriOverride, brokerUriOverride));
    ...
    public static IHostBuilder ConfigureAgienceHost(this IHostBuilder hostBuilder)
    {
        hostBuilder.ConfigureServices((context, services) =>
        {
            ...
            var authorityUriOverride = configuration["AuthorityUriOverride"];
            var brokerUriOverride = configuration["BrokerUriOverride"];
            ...
            services.AddSingleton(x => new Authority(authorityUri, x.GetRequiredService<Broker>(), x.GetRequiredService<ILogger<Authority>>(), authorityUriOverride, brokerUriOverride));
    ...
    {
        appBuilder.Services.AddSingleton(x => new Broker(x.GetRequiredService<ILogger<Broker>>(), customNtpHost));
        appBuilder.Services.AddSingleton(x => new Authority(authorityUri, x.GetRequiredService<Broker>(), x.GetRequiredService<IAuthorityDataAdapter>(), x.GetRequiredService<ILogger<Authority>>(), authorityUriOverride, brokerUriOverride));
        return appBuilder;
    }
}
