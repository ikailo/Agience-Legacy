...
public static class HostBuilderExtensions
{
          ...
          )
    {
        ...
        appBuilder.Services.AddSingleton(x => new Authority(authorityUri, x.GetRequiredService<Broker>(), x.GetRequiredService<ILogger<Authority>>(), authorityUriInternal, brokerUriInternal));
    ...
    public static IHostBuilder ConfigureAgienceHost(this IHostBuilder hostBuilder)
    {
        hostBuilder.ConfigureServices((context, services) =>
        {
            ...
            var authorityUriInternal = configuration["AuthorityUriInternal"];
            var brokerUriInternal = configuration["BrokerUriInternal"];
            ...
            services.AddSingleton(x => new Authority(authorityUri, x.GetRequiredService<Broker>(), x.GetRequiredService<ILogger<Authority>>(), authorityUriInternal, brokerUriInternal));
    ...
    {
        appBuilder.Services.AddSingleton(x => new Broker(x.GetRequiredService<ILogger<Broker>>(), customNtpHost));
        appBuilder.Services.AddSingleton(x => new Authority(authorityUri, x.GetRequiredService<Broker>(), x.GetRequiredService<IAuthorityDataAdapter>(), x.GetRequiredService<ILogger<Authority>>(), authorityUriInternal, brokerUriInternal));
        return appBuilder;
    }
}
